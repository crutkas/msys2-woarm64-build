...
def collect(command, output, abi, timeout, read_context=True):
    api = Api()
    in_job = W.BOOL()
    ...
    try:
        ...
        receipt = {"schema": 1, "status": "experimental-raw-only" if failure is None else "collector-failed",
                   ...
                   "debug_mode": "DEBUG_PROCESS", "memory_writes": 0, "register_writes": 0,
...
def main():
    ...
    collect(command, args.output, json.loads(args.abi.read_text()), args.timeout, not args.no_context)

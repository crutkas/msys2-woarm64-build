...
def run_case(name, root_name="private", suffix="01"):
    ...
    for process in capture["processes"]:
        ...
        if len(runtimes) != 1 or runtimes[0].sha256 != RUNTIME_SHA or runtimes[0].path != (bin_dir / "msys-2.0.dll").resolve():
    ...
    target_executed = any(Path(row["executable"]).stem.lower() == name for row in exits) if name != "pipeline" else all(

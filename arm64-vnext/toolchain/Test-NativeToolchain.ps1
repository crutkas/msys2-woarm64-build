...
$assembler = Tool as
if ($Profile -eq 'MSYS') {
    $selectedAssembler = (& $cc '-print-prog-name=as' | Out-String).Trim()
    if ($LASTEXITCODE -ne 0 -or
        -not (Test-Path -LiteralPath $selectedAssembler -PathType Leaf)) {
        throw 'MSYS compiler did not resolve its target assembler.'
    }
    $assembler = [IO.Path]::GetFullPath($selectedAssembler)
}
$null = Tool ld
$null = Tool ar
$null = Tool nm

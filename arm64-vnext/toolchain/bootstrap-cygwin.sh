...
case $STAGE in
  ...
  w32api)
    ...
    overlay=$ROOT/build/w32api-source
    patch_sha=$(sha256sum "$RECIPE/w32api-arm64-cygwin.patch" | cut -d' ' -f1)
    if [[ ! -d $overlay ]]; then
      mkdir "$overlay"
      git -C "$ROOT/sources/mingw-w64" archive HEAD | tar -x -C "$overlay"
      (cd "$overlay" && git apply --check "$RECIPE/w32api-arm64-cygwin.patch" &&
        git apply "$RECIPE/w32api-arm64-cygwin.patch")
      printf '%s\n' "$patch_sha" > "$overlay/recipe-patch.sha256"
    fi
    [[ -f $overlay/recipe-patch.sha256 &&
       $(<"$overlay/recipe-patch.sha256") == "$patch_sha" ]] ||
      fail "Incomplete or different w32api overlay: inspect $overlay"
  ...
  w32api-libs)
    ...
    for library in kernel32 ntdll user32 winmm advapi32 netapi32 shell32 \
                   ole32 oleaut32 ws2_32 crypt32 secur32; do

...
class BoundedProcessControls(unittest.TestCase):
    def execute(self, code, timeout=10, **kwargs):
        with tempfile.TemporaryDirectory(prefix="native-job-control-") as root:
            with (Path(root) / "output.log").open("xb") as log:
                return run([sys.executable, "-c", code], cwd=root, env=dict(os.environ),
                           log=log, timeout=timeout, **kwargs)
    ...
    def test_explicit_drain_grace_waits_without_waiving_orphans(self):
        result = self.execute(
            "import subprocess,sys; subprocess.Popen([sys.executable,'-c','import time; time.sleep(0.8)'])",
            drain_grace=3)
        self.assertTrue(result["passed"])
        self.assertEqual(result["active_at_boundary"], 0)
        self.assertEqual(result["drain_grace_seconds"], 3)

    def test_drain_grace_is_bounded(self):
        with self.assertRaises(ValueError):
            self.execute("print('invalid grace')", drain_grace=11)

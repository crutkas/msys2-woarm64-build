...
exits=$(cygpath -u "${2:?Existing private relay directory required}")
[[ $exits == "$root/"* && -d $exits ]] || { echo "Existing private relay directory required" >&2; return 2; }
export WOARM64_NATIVE_EXIT_DIR="$(cygpath -w "$exits")"
case "$1" in
    build)

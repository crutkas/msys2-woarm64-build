...
int main(int argc, char **argv)
{
    ...
    int edit_mode = argc == 4 && strcmp(argv[3], "libedit") == 0;
    int library_mode = argc == 4 && (strcmp(argv[3], "library") == 0 || edit_mode);
    int history_mode = argc == 4 && strcmp(argv[3], "history") == 0;
    ...
    if (child == 0) {
        ...
        if (slave > 2)
            close(slave);
        if (curses_mode || library_mode || history_mode)
    ...
    if (await_text(master, log, curses_mode ? "curses-cpp-ready" :
                   history_mode ? "history-api-ready" :
                   library_mode ? "chain-ready> " : "before")) {
        ...
        if (ready) {
            ...
            if (curses_mode || history_mode)
                interacted = interacted && write(master, "\r", 1) == 1;
            else if (library_mode) {
                const char *edit = edit_mode ?
                    "before \316\273\344\270\255X\177\177\177\316\273\344\270\255\r" :
                    "before \316\273X\177\177\316\273\r";
                size_t edit_length = strlen(edit);
                interacted = interacted &&
                    write(master, edit, edit_length) == (ssize_t)edit_length &&
                    await_text(master, log, "chain-history> ") &&
                    write(master, "\020\r", 2) == 2;
            }
            else

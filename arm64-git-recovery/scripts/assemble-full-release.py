...
def generic_packages(export_path: Path, export: dict, role: str) -> list[dict]:
    ...
    for item in packages:
        if not isinstance(item, dict):
            raise ContractError(f"Provider export has invalid package entry: {export_path.name}")
        path_value = item.get("path")
        archive_value = item.get("archive")
        if path_value and archive_value and path_value != archive_value:
            raise ContractError(
                f"Provider package path aliases disagree: {export_path.name}"
            )
        path_value = path_value or archive_value
        if not isinstance(path_value, str) or not path_value:
            raise ContractError(f"Provider package has no path: {export_path.name}")
        ...
        if declared_name and package_name and declared_name != package_name:
            raise ContractError(
                f"Provider package name aliases disagree: {export_path.name}"
            )
        declared_name = declared_name or package_name
        if not isinstance(declared_name, str) or not declared_name.startswith(
            ("mingw-w64-", "msys2-runtime")
        ):
            declared_name = None
        archive_hash = item.get("sha256")
        if not isinstance(archive_hash, str) or len(archive_hash) != 64:
            raise ContractError(f"Provider package has invalid SHA-256: {export_path.name}")
        result.append({
            "name": declared_name,
            "path": package_path,
            "sha256": archive_hash,
...
def git_package_identity(item: dict) -> tuple[str, str, str]:
    ...
    return name, path, archive_hash.lower()
...
def resolve_git_source_identity(git_path: Path, git: dict) -> tuple[dict, list[dict]]:
    ...
    if changed:
        ...
        if changed != ["mingw-w64-aarch64-git-p4"]:
            raise ContractError(
                "Corrected Git handoff changed packages other than git-p4"
            )
        if (
    ...
    source["recipeSha256"] = recipe_hash
    return source, [{
...
def collect_declared_packages(input_path: Path, input_data: dict,
    ...
    if contract.get("source_identity"):
        git_source, provenance_references = resolve_git_source_identity(git_path, git)
...
def resolve_packages(packages: dict[str, dict], roots: list[str],
    ...
    for name in selected:
        conflicts = {dependency_name(value) for value in packages[name]["conflicts"]}
        for other in selected:
            if other == name:
                continue
            provided = {other, *(dependency_name(v) for v in packages[other]["provides"])}
            if conflicts & provided:
                raise ContractError(f"Selected package conflict: {name} conflicts with {other}")
    ...
    while unresolved:
        ...
        for rel, link in list(unresolved.items()):
            if link["kind"] == "hardlink":
                ...
                candidates = [
                    safe_relative((PurePosixPath(rel).parent / link["target"]).as_posix()),
                    safe_relative(link["target"]),
                ]

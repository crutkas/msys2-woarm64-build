...
def require_arm64_pe(path):
    ...
    if (
        ...
        raise ValueError(
            "Native target execution requires ordinary ARM64 PE32+, not a DLL"
        )
...
def main():
    if os.name != "nt" or len(sys.argv) < 2:
        raise SystemExit(
            "Native Windows execution and an explicit target command are required"
        )
    ...
    root = Path(windows_path(os.environ["WOARM64_NATIVE_TEST_ROOT"])).resolve(
        strict=True
    )
    ...
    if not executable.is_relative_to(root) or executable.suffix.lower() != ".exe":
        raise SystemExit(
            "Only an explicit executable inside the owned native test root may run"
        )
    if re.fullmatch(
        r"(?:.*-)?(?:gcc|g\+\+|cc|c\+\+|cc1|cc1plus|collect2|as|ld|ar|ranlib)(?:\.exe)?",
        executable.name,
    ):
        raise SystemExit("This relay is for target tests, not compiler/tool proxies")
    require_arm64_pe(executable)
    log_root = Path(
        windows_path(os.environ["WOARM64_NATIVE_EXIT_DIR"])
    ).resolve(strict=True)
    ...
    with subprocess.Popen([str(executable), *arguments]) as process:
        ...
        get_times.argtypes = [wintypes.HANDLE] + [
            ctypes.POINTER(wintypes.FILETIME)
        ] * 4
        get_times.restype = wintypes.BOOL
        created, ended, kernel_time, user_time = (
    ...
    with (
        log_root / f"{os.getpid()}-{uuid.uuid4().hex}.json"
    ).open("x", encoding="utf-8") as stream:

    ...
    for identity, record in records.items():
        ...
        for candidate in relayed.get(identity, []):
            ...
            if (contract is not None
                    ...
                    == str(contract.get("image_name", "")).casefold()
                    and normalise_windows_path(record["executable"])
                    == normalise_windows_path(parent_record["executable"])):
                protected.append({
                    ...
                    "classification": "generation-bound-expected-probe-exit",
                })
                break
    return protected
...
def run(command, cwd, target_root, relay_records, log_path, timeout):
    ...
    contract_path = Path(__file__).with_name("expected-exit-contracts.json")
    expected_contracts = {}
    if contract_path.is_file():
        expected_contracts = json.loads(contract_path.read_text(encoding="utf-8")).get("contracts", {})

...
if ($output.StartsWith("$source\", [StringComparison]::OrdinalIgnoreCase)) {
    throw 'Prepared recipe must be outside its source.'
}
...
$members = @($inventory.files.GetEnumerator() | Where-Object {
    $_.Key.StartsWith("$name/", [StringComparison]::Ordinal)
} | ForEach-Object {
...
if ($BootstrapAutotools) {
    ...
    if ([regex]::Matches($text,[regex]::Escape($aggregate)).Count -ne 1) {
        throw 'Expected one autotools build-dependency aggregate.'
    }

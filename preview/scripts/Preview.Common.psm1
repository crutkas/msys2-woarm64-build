...
function Test-PreviewOrdinalEqual {
    param(
        [AllowNull()] $Left,
        [AllowNull()] $Right
    )
...
function Test-PreviewOrdinalIgnoreCaseEqual {
    param(
        [AllowNull()] $Left,
        [AllowNull()] $Right
    )
...
function Test-PreviewOrdinalIn {
    param(
        [AllowNull()] $Value,
        [Parameter(Mandatory = $true)][object[]] $Candidates
    )
    foreach ($candidate in $Candidates) {
        if (Test-PreviewOrdinalEqual -Left $Value -Right $candidate) {
            return $true
        }
    }
    return $false
...
function Test-PreviewOrdinalIgnoreCaseIn {
    param(
        [AllowNull()] $Value,
        [Parameter(Mandatory = $true)][object[]] $Candidates
    )
    foreach ($candidate in $Candidates) {
        if (Test-PreviewOrdinalIgnoreCaseEqual -Left $Value -Right $candidate) {
            return $true
        }
    }
    return $false
...
function Assert-PrivatePath {
    ...
    while (-not [string]::IsNullOrWhiteSpace($cursor)) {
        ...
        if ([string]::IsNullOrWhiteSpace($parent) -or
            (Test-PreviewOrdinalIgnoreCaseEqual -Left $parent -Right $cursor)) {
            break
        }
        $cursor = $parent
    }
...
function Assert-PreviewLock {
    ...
    foreach ($closurePath in @($Lock.nativeShellClosure)) {
        ...
        if (-not (Test-PreviewOrdinalEqual -Left $safeClosurePath `
                -Right ([string]$closurePath)) -or
            -not $closurePaths.Add($safeClosurePath)) {
            throw "Native shell closure contains an unsafe or duplicate path: '$closurePath'"
        }
    }
    if (@($Lock.inputs | Where-Object {
                Test-PreviewOrdinalEqual -Left $_.role -Right 'base'
            }).Count -ne 1) {
    ...
    foreach ($required in $requiredInputs.GetEnumerator()) {
        $matches = @($Lock.inputs | Where-Object {
                Test-PreviewOrdinalEqual -Left $_.id -Right $required.Key
            })
        if ($matches.Count -ne 1 -or
            -not (Test-PreviewOrdinalEqual -Left $matches[0].role -Right $required.Value)) {
    ...
    foreach ($input in @($Lock.inputs)) {
        ...
        if ($input.overlay.enabled -and
            -not (Test-PreviewOrdinalIn -Value $input.role -Candidates $payloadRoles)) {
        ...
        if ((Test-PreviewOrdinalEqual -Left $input.status -Right 'resolved') -and
            $input.overlay.enabled -and
        ...
        foreach ($mapping in @($input.overlay.mappings)) {
            ...
            if (-not (Test-PreviewOrdinalEqual -Left $source `
                    -Right ([string]$mapping.source)) -or
                -not (Test-PreviewOrdinalEqual -Left $destination `
                    -Right ([string]$mapping.destination))) {
        ...
        if (-not (Test-PreviewOrdinalIn -Value $input.status `
        ...
        if (-not (Test-PreviewOrdinalEqual -Left $repository -Right 'git-for-windows/git') -and
            -not (Test-PreviewOrdinalEqual -Left $repository -Right 'ip7z/7zip') -and
        ...
        if ([string]$input.asset.sha256 -notmatch '^[0-9a-f]{64}$' -or
            (Test-PreviewOrdinalEqual -Left ([string]$input.asset.sha256) `
                -Right ('0' * 64))) {
        ...
        if ($null -ne $sourcePathProperty -and
            ...
            if (-not (Test-PreviewOrdinalEqual -Left $safeSourcePath `
                    -Right ([string]$sourcePathProperty.Value)) -or
                -not (Test-PreviewOrdinalEqual -Left ([string]$input.asset.url) `
                    -Right $expectedRawUrl) -or
                -not (Test-PreviewOrdinalEqual -Left ([string]$input.asset.name) `
                    -Right ([IO.Path]::GetFileName($safeSourcePath)))) {
            ...
            if (-not (Test-PreviewOrdinalEqual -Left ([string]$input.asset.url) `
                    -Right $expectedReleaseUrl)) {
        ...
        if ($null -ne $input.asset.PSObject.Properties['members']) {
            ...
            foreach ($member in @($input.asset.members)) {
                ...
                if ([Int64]$member.expectedBytes -le 0 -or
                    ...
                    (Test-PreviewOrdinalEqual -Left ([string]$member.sha256) `
                        -Right ('0' * 64))) {
        ...
        if (([string]$input.role).EndsWith('package', [StringComparison]::Ordinal) -and
            $null -eq $input.package) {
...
function Get-VerifiedInput {
    ...
    if (Test-Path -LiteralPath $destination) {
        ...
        if ($actualBytes -ne [Int64]$InputEntry.asset.expectedBytes -or
            -not (Test-PreviewOrdinalEqual -Left $actualHash `
    ...
    try {
        ...
        if (-not (Test-PreviewOrdinalEqual -Left $actualHash `
                -Right ([string]$InputEntry.asset.sha256))) {
...
function ConvertTo-SafeArchivePath {
    ...
    if ([string]::IsNullOrWhiteSpace($path) -or
        (Test-PreviewOrdinalEqual -Left $path -Right '.')) {
        return '.'
    }
    ...
    foreach ($segment in $path.Split('/')) {
        if ([string]::IsNullOrEmpty($segment) -or
            (Test-PreviewOrdinalEqual -Left $segment -Right '.')) {
            continue
        }
        if (Test-PreviewOrdinalEqual -Left $segment -Right '..') {
...
function Assert-ArchiveMemberNames {
    ...
    foreach ($raw in $Members) {
        ...
        if (Test-PreviewOrdinalEqual -Left $normalized -Right '.') {
            continue
        }
...
function Get-ArchiveMembers {
    ...
    foreach ($line in $lines) {
        $text = [string]$line
        if (Test-PreviewOrdinalEqual -Left $text -Right '----------') {
            $inEntries = $true
            continue
        }
        if (-not $inEntries) {
    ...
    $entryBlocks = @($blocks | Where-Object {
        -not (Test-PreviewOrdinalEqual `
            -Left (ConvertTo-SafeArchivePath -Member ([string]$_.Path)) -Right '.')
    })
    ...
    for ($index = 0; $index -lt $entryBlocks.Count; $index++) {
        ...
        $type = if (-not [string]::IsNullOrWhiteSpace($symbolicLink)) {
            ...
            'hardlink'
        } elseif ($block.ContainsKey('Folder') -and
            (Test-PreviewOrdinalEqual -Left $block.Folder -Right '+')) {
            'directory'
        } else {
            'file'
        }
        $members.Add([pscustomobject]@{
            ...
            type = $type
            target = if (Test-PreviewOrdinalEqual -Left $type -Right 'symlink') {
                $symbolicLink
            }
            elseif (Test-PreviewOrdinalEqual -Left $type -Right 'hardlink') {
                $hardLink
            }
            else {
                $null
...
function Resolve-ArchiveLinkTarget {
    ...
    if (Test-PreviewOrdinalEqual -Left $Member.type -Right 'hardlink') {
...
function Assert-ArchiveLinks {
    ...
    foreach ($member in $members | Where-Object {
            Test-PreviewOrdinalIn -Value $_.type -Candidates @('symlink', 'hardlink')
        }) {
        ...
        if ((Test-PreviewOrdinalEqual -Left $member.type -Right 'hardlink') -and
            -not (Test-PreviewOrdinalEqual `
                -Left $memberByPath[$targetPath].type -Right 'file')) {
            throw "Archive hard link '$($member.path)' does not target a regular archive member"
        }
    }

    foreach ($member in $members | Where-Object {
            Test-PreviewOrdinalEqual -Left $_.type -Right 'symlink'
        }) {
        $visited = [Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        $cursor = $member
        while (Test-PreviewOrdinalEqual -Left $cursor.type -Right 'symlink') {
        ...
        if (-not (Test-PreviewOrdinalEqual -Left $cursor.type -Right 'file')) {
...
function Expand-VerifiedArchive {
    ...
    try {
        ...
        foreach ($member in $members) {
            ...
            if ((Test-PreviewOrdinalEqual -Left $member.type -Right 'directory') -and
                -not $item.PSIsContainer) {
                throw "Archive directory changed type during extraction: $($member.path)"
            }
            if ((Test-PreviewOrdinalEqual -Left $member.type -Right 'symlink') -and
                (-not ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -or
                -not (Test-PreviewOrdinalEqual `
                    -Left ([string]$item.LinkTarget.Replace('\', '/')) `
                    -Right ([string]$member.target.Replace('\', '/'))))) {
                throw "Archive symlink was not preserved exactly: $($member.path)"
            }
            if ((Test-PreviewOrdinalEqual -Left $member.type -Right 'hardlink') -and
                -not (Test-PreviewOrdinalEqual -Left $item.LinkType -Right 'HardLink')) {
                throw "Archive hard link was not preserved: $($member.path)"
            }
            if (-not (Test-PreviewOrdinalEqual -Left $member.type -Right 'directory')) {
...
function Invoke-PseudoRelocScanner {
    ...
    if (-not (Test-PreviewOrdinalEqual -Left $result.result -Right 'pass') -or
        @($result.policy_violations).Count -ne 0) {
...
function Assert-EvidenceComplete {
    ...
    foreach ($testId in @(
        ...
        if (@($Evidence.tests | Where-Object {
                    Test-PreviewOrdinalEqual -Left $_.id -Right $testId
                }).Count -ne 1) {
            throw "Validation evidence is missing test '$testId'"
        }
    }
    if (@($Evidence.artifacts | Where-Object {
                Test-PreviewOrdinalIn -Value $_.kind `
                    -Candidates @('lock', 'provenance', 'payload')
            }).Count -ne 3) {
...
function Assert-NoPreparationTools {
    ...
    $leaks = @(Get-ChildItem -LiteralPath $Root -File -Force -Recurse |
        Where-Object {
            (Test-PreviewOrdinalIgnoreCaseIn -Value $_.Name -Candidates $forbiddenNames) -and
...
Export-ModuleMember -Function @(
    ...
    'Read-PreviewLock',
    'Test-PreviewOrdinalEqual',
    'Test-PreviewOrdinalIgnoreCaseEqual',
    'Test-PreviewOrdinalIn',
    'Test-PreviewOrdinalIgnoreCaseIn'
)

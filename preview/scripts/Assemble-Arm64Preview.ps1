...
function Test-SharedObservationEqual {
    ...
    return Test-PreviewOrdinalEqual `
        -Left ($Before | ConvertTo-Json -Compress -Depth 8) `
        -Right ($After | ConvertTo-Json -Compress -Depth 8)
}
...
function Initialize-PrivateCache {
    ...
    if (-not (Test-PreviewOrdinalEqual `
            -Left $markerValue -Right 'portable-git-arm64-preview-cache-v1')) {
...
function Copy-GitBlob {
    ...
    if (-not (Test-PreviewOrdinalEqual `
            -Left (ConvertTo-SafeArchivePath -Member $gitPath) -Right $gitPath)) {
...
function Get-VerifiedTreeMembers {
    ...
    foreach ($archiveMember in $ArchiveMembers) {
        ...
        $byPath[$path] = [ordered]@{
            path = $path
            type = $type
            bytes = if (Test-PreviewOrdinalEqual -Left $type -Right 'directory') {
                0
            } else {
                $item.Length
            }
            sha256 = if (Test-PreviewOrdinalEqual -Left $type -Right 'directory') {
                $null
            } else {
                Get-Sha256 -Path $item.FullName
            }
            linkTarget = if (Test-PreviewOrdinalIn `
                    -Value $type -Candidates @('symlink', 'hardlink')) {
...
function Resolve-ManifestLinkTarget {
    ...
    $candidate = if ((Test-PreviewOrdinalEqual -Left $Type -Right 'hardlink') -or
...
function Copy-VerifiedRawInput {
    ...
    if ((Get-Item -LiteralPath $destination).Length -ne
        [Int64]$InputEntry.asset.expectedBytes -or
        -not (Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $destination) `
            -Right ([string]$InputEntry.asset.sha256))) {
...
function Get-OverlaySelection {
    ...
    foreach ($mapping in @($InputEntry.overlay.mappings)) {
        ...
        $sourceMembers = if (Test-PreviewOrdinalEqual -Left $source -Right '.') {
            @($ArchiveMembers)
        } else {
            ...
            if (Test-PreviewOrdinalEqual -Left ([string]$sourceEntry.type) -Right 'directory') {
                @($ArchiveMembers | Where-Object {
                    (Test-PreviewOrdinalEqual -Left ([string]$_.path) -Right $source) -or
        ...
        foreach ($member in $sourceMembers) {
            ...
            $suffix = if (Test-PreviewOrdinalEqual -Left $source -Right '.') {
                $sourceMember
            } elseif (Test-PreviewOrdinalEqual -Left $sourceMember -Right $source) {
                ''
            } else {
                $sourceMember.Substring($source.Length + 1)
            }
            $destinationPath = if (Test-PreviewOrdinalEqual -Left $destination -Right '.') {
...
function Write-CanonicalBundleLock {
    ...
    $canonicalInputs = @($inputIds | ForEach-Object {
        $inputId = $_
        $input = @($SourceLock.inputs | Where-Object {
                Test-PreviewOrdinalEqual -Left $_.id -Right $inputId
            })[0]
        if (Test-PreviewOrdinalEqual -Left $input.status -Right 'unresolved') {
            return [ordered]@{
                id = $input.id
                role = if (Test-PreviewOrdinalEqual `
                        -Left $input.role -Right 'native-shell-package') {
                    'payload'
                } else {
                    'validation-tool'
                }
        ...
        $canonicalRole = if (Test-PreviewOrdinalEqual -Left $input.role -Right 'base') {
            'base-bundle'
        ...
        if ($null -ne $input.package -and
            (Test-PreviewOrdinalEqual -Left $canonicalRole -Right 'payload')) {
            $canonicalPackage = [ordered]@{
                name = $input.package.name
                version = $input.package.version
                personality = if (Test-PreviewOrdinalEqual `
                        -Left $canonicalRole -Right 'validation-tool') {
                    'tool'
...
function Assert-PackageMetadata {
    ...
    if ($name.Count -ne 1 -or
        -not (Test-PreviewOrdinalEqual -Left $name[0] -Right $InputEntry.package.name)) {
        throw "Package name mismatch for '$($InputEntry.id)'"
    }

    if ($version.Count -ne 1 -or
        -not (Test-PreviewOrdinalEqual -Left $version[0] -Right $InputEntry.package.version)) {
    ...
    if (-not (Test-PreviewOrdinalEqual `
            -Left ($expectedProvides -join "`n") -Right ($actualProvides -join "`n"))) {
...
function Assert-PinnedMembers {
    ...
    foreach ($member in @($InputEntry.asset.members)) {
        ...
        if ($bytes -ne [Int64]$member.expectedBytes -or
            -not (Test-PreviewOrdinalEqual -Left $hash -Right ([string]$member.sha256))) {
...
function Copy-OverlayMapping {
    ...
    $source = if (Test-PreviewOrdinalEqual -Left $safeSource -Right '.') {
        $StagingRoot
    } else {
        Join-Path $StagingRoot $safeSource.Replace('/', '\')
    }
    $destination = if (Test-PreviewOrdinalEqual -Left $safeDestination -Right '.') {
        $PortableRoot
    } else {
        Join-Path $PortableRoot $safeDestination.Replace('/', '\')
    }
    ...
    foreach ($item in $items) {
        ...
        $target = if ([string]::IsNullOrEmpty($relative) -or
            (Test-PreviewOrdinalEqual -Left $relative -Right '.')) {
            $destination
        } else {
        ...
        if ($item.PSIsContainer -and -not ($item.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
            ...
            if ($exists -and
                -not (Test-PreviewOrdinalEqual -Left $payloadPath -Right '.') -and
                -not [bool]$Mapping.allowOverwrite) {
            ...
            if ($exists -and
                -not (Test-PreviewOrdinalEqual -Left $payloadPath -Right '.')) {
        ...
        if (Test-PreviewOrdinalEqual -Left $item.LinkType -Right 'HardLink') {
...
function Get-MaterializedMemberIdentity {
    ...
    if (-not (Test-PreviewOrdinalEqual -Left $Item.LinkType -Right 'HardLink')) {
    ...
    if (Test-PreviewOrdinalEqual -Left $memberPath -Right $ordered[0]) {
        return [ordered]@{ type = 'file'; linkTarget = $null }
    }
...
function Assert-PayloadManifestUnchanged {
    ...
    foreach ($item in Get-ChildItem -LiteralPath $PortableRoot -Force -Recurse |
        ...
        $type = $identity.type
        if (-not (Test-PreviewOrdinalEqual -Left ([string]$entry.type) -Right $type)) {
            throw "Payload changed type after manifest generation: $relative"
        }
        if (-not (Test-PreviewOrdinalEqual -Left ([string]$entry.linkTarget) `
                -Right ([string]$identity.linkTarget))) {
            throw "Payload changed link target after manifest generation: $relative"
        }
        if (-not (Test-PreviewOrdinalEqual -Left $type -Right 'directory') -and
            ($item.Length -ne [Int64]$entry.bytes -or
            -not (Test-PreviewOrdinalEqual -Left (Get-Sha256 $item.FullName) `
...
function Get-ToolTreeSnapshot {
    ...
    return @(Sort-ObjectsOrdinal -Items $unsorted -Properties @('path'))
}
...
for ($left = 0; $left -lt $privatePaths.Count; $left++) {
    ...
    if (-not (Test-PreviewOrdinalIgnoreCaseEqual `
            -Left ([IO.Path]::GetPathRoot($privatePaths[0])) `
            -Right ([IO.Path]::GetPathRoot($privatePaths[2])))) {
...
if ($lockRepositoryPath.StartsWith('../', [StringComparison]::Ordinal) -or
    (Test-PreviewOrdinalEqual -Left $lockRepositoryPath -Right '..')) {
...
$ownership = @{}
$base = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'portable-git-arm64-base'
    })[0]
$scannerHostInput = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'scanner-host-portable-git-x64'
    })[0]
$extractorBootstrap = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'archive-extractor-7zr'
    })[0]
$extractorBundle = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'archive-extractor-7zip-extra'
    })[0]
...
foreach ($input in @($lock.inputs | Where-Object {
            $_.overlay.enabled -and
            -not (Test-PreviewOrdinalEqual -Left $_.role -Right 'base')
        })) {
...
if (-not (Test-PreviewOrdinalEqual `
        -Left ($committedLock | ConvertTo-Json -Compress -Depth 20) `
        -Right ($lock | ConvertTo-Json -Compress -Depth 20))) {
...
$binutilsInput = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'fixed-binutils'
    })[0]
$scannerInput = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.id -Right 'fixed-pseudo-reloc-scanner'
    })[0]
...
$inputEvidence = @($canonicalLock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.status -Right 'resolved'
    } | ForEach-Object {
    $canonicalInput = $_
    ...
    $archiveMembers = @($archiveMembersByInput[[string]$canonicalInput.id] | ForEach-Object {
        ...
        [ordered]@{
            ...
            type = $member.type
            bytes = if (Test-PreviewOrdinalEqual -Left $member.type -Right 'directory') {
                0
            } else {
                [Int64]$member.bytes
            }
...
$unsortedFinalMembers = @($ownership.GetEnumerator() | Where-Object {
        -not (Test-PreviewOrdinalEqual -Left $_.Key -Right '.')
    } | ForEach-Object {
    ...
    [ordered]@{
        ...
        type = $memberType
        bytes = if (Test-PreviewOrdinalEqual -Left $memberType -Right 'directory') {
            0
        } else {
            [Int64]$member.Length
        }
        sha256 = if (Test-PreviewOrdinalEqual -Left $memberType -Right 'directory') {
            $null
        } else {
            Get-Sha256 -Path $member.FullName
        }
        linkTarget = $identity.linkTarget
    }
})
...
$pseudoRelocCandidates = @($finalMembers | Where-Object {
        (Test-PreviewOrdinalIn -Value $_.type -Candidates @('file', 'hardlink')) -and
        $_.destinationPath -match '(?i)\.(?:exe|dll)$'
    } | ForEach-Object {
        [ordered]@{
            destinationPath = $_.destinationPath
            inputId = $_.inputId
...
foreach ($validatorInput in @($lock.inputs | Where-Object {
            Test-PreviewOrdinalIn -Value $_.role -Candidates @(
...
foreach ($collectorInput in @($lock.inputs | Where-Object {
            Test-PreviewOrdinalIn -Value $_.role `
                -Candidates @('authoritative-runtime-collector', 'authoritative-runtime-support')
        })) {
...
$validatorMainInputs = @($lock.inputs | Where-Object {
        Test-PreviewOrdinalEqual -Left $_.role -Right 'authoritative-validator-main'
    })
...
if (-not (Test-Path -LiteralPath $authoritativeValidatorPath -PathType Leaf) -or
    -not (Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $authoritativeValidatorPath) `
        -Right ([string]$validatorMainInputs[0].asset.sha256))) {
...
if (-not (Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $bundleLockPath) `
        -Right $preValidatorArtifactHashes.lock) -or
    -not (Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $provenancePath) `
        -Right $preValidatorArtifactHashes.provenance) -or
    -not (Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $payloadPath) `
        -Right $preValidatorArtifactHashes.payload)) {
...
if (-not (Test-PreviewOrdinalEqual `
        -Left ($postValidatorToolSnapshot | ConvertTo-Json -Compress -Depth 8) `
        -Right ($preValidatorToolSnapshot | ConvertTo-Json -Compress -Depth 8))) {
...
if ($null -eq $authoritativeEvidence.PSObject.Properties['schemaVersion'] -or
    ...
    -not (Test-PreviewOrdinalEqual -Left $authoritativeEvidence.result -Right 'pass')) {

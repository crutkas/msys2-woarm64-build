...
function Assert-Equal {
    ...
    $equal = if ($Expected -is [string] -or $Actual -is [string]) {
        Test-PreviewOrdinalEqual -Left $Expected -Right $Actual
    }
    else {
        $Expected -eq $Actual
    }
...
$tempRoot = if (-not [string]::IsNullOrWhiteSpace($env:RUNNER_TEMP)) {
    (Get-Item -LiteralPath $env:RUNNER_TEMP).FullName
}
else {
    (Get-Item -LiteralPath ([IO.Path]::GetTempPath())).FullName
}
$temporary = Join-Path $tempRoot "arm64-preview-tests-$([Guid]::NewGuid().ToString('N'))"
...
try {
    ...
    Invoke-Test 'immutable source identities stay on the approved forks' {
        ...
        Assert-Equal $false (
            Test-PreviewOrdinalEqual -Left "re${softHyphen}sume" -Right 'resume'
        ) 'Soft hyphens must not collapse under security comparisons.'
        $candidate = Copy-Object $lock
        $candidate.inputs[0].identity.repository = "git-for-windows/g${softHyphen}it"
        Assert-Throws { Assert-PreviewLock -Lock $candidate } 'outside the immutable allowlist'
        ...
        try {
            ...
            Assert-Equal $false (
                Test-PreviewOrdinalEqual -Left "f$([char]0x0130)xed-binutils" `
                    -Right 'fixed-binutils'
            ) 'Turkish-I must remain distinct under ordinal comparison.'
            $candidate = Copy-Object $lock
            $fixed = @($candidate.inputs | Where-Object {
                    Test-PreviewOrdinalEqual -Left $_.id -Right 'fixed-binutils'
                })[0]
            $fixed.id = "f$([char]0x0130)xed-binutils"
            Assert-Throws { Assert-PreviewLock -Lock $candidate } `
                'must contain exactly one.*fixed-binutils'
        }
        finally {
    ...
    Invoke-Test 'shared-root and alias paths are rejected' {
        ...
        $sealScript = Join-Path $scripts 'Collect-AdmissionEvidence.ps1'
        $sealRoot = Join-Path $temporary 'shared-root-seal'
        $beforeSeal = Join-Path $temporary 'shared-root-before.json'
        $sameSeal = Join-Path $temporary 'shared-root-same.json'
        $changedSeal = Join-Path $temporary 'shared-root-changed.json'
        $sealLog = Join-Path $sealRoot 'var\log\pacman.log'
        $sealDatabase = Join-Path $sealRoot 'var\lib\pacman\local\fixture'
        New-Item -ItemType Directory -Path $sealDatabase -Force | Out-Null
        New-Item -ItemType Directory -Path (Split-Path -Parent $sealLog) -Force | Out-Null
        Set-Content -LiteralPath (Join-Path $sealDatabase 'desc') `
            -Value 'fixture' -NoNewline -Encoding ascii
        Set-Content -LiteralPath $sealLog `
            -Value 'before' -NoNewline -Encoding ascii
        & pwsh -NoLogo -NoProfile -File $sealScript -CaptureSharedRootSeal `
            -SharedRootPath $sealRoot -SealOutputPath $beforeSeal
        Assert-Equal 0 $LASTEXITCODE ''
        & pwsh -NoLogo -NoProfile -File $sealScript -CaptureSharedRootSeal `
            -SharedRootPath $sealRoot -SealOutputPath $sameSeal
        Assert-Equal 0 $LASTEXITCODE ''
        Assert-Equal (Get-Sha256 -Path $beforeSeal) (Get-Sha256 -Path $sameSeal) `
            'Unchanged shared roots must have deterministic seals.'
        Set-Content -LiteralPath $sealLog `
            -Value 'after' -NoNewline -Encoding ascii
        & pwsh -NoLogo -NoProfile -File $sealScript -CaptureSharedRootSeal `
            -SharedRootPath $sealRoot -SealOutputPath $changedSeal
        Assert-Equal 0 $LASTEXITCODE ''
        Assert-Equal $false (
            Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $beforeSeal) `
                -Right (Get-Sha256 -Path $changedSeal)
        ) 'A shared-root mutation must change its seal.'
    }
...
'@ | Set-Content -LiteralPath $scanner -Encoding utf8
    ...
    Invoke-Test 'immutable artifact bindings are acyclic' {
        ...
        $commonSource = Get-Content -LiteralPath (Join-Path $scripts 'Preview.Common.psm1') -Raw
        $workflowSource = Get-Content -LiteralPath (
            Join-Path $PSScriptRoot '..\..\.github\workflows\preview-harness.yml'
        ) -Raw
        $admissionSource = Get-Content -LiteralPath (
            Join-Path $scripts 'Collect-AdmissionEvidence.ps1'
        ) -Raw
        $admissionSchema = Get-Content -LiteralPath (
            Join-Path $PSScriptRoot '..\schemas\admission-evidence.schema.json'
        ...
        Assert-Equal $true $assemblerSource.Contains(
            'Test-PreviewOrdinalEqual -Left (Get-Sha256 -Path $authoritativeValidatorPath)') ''
        Assert-Equal $false ([regex]::IsMatch($assemblerSource, '-c(?:eq|ne)\b')) ''
        Assert-Equal $false ([regex]::IsMatch($commonSource, '-c(?:eq|ne)\b')) ''
        $actionUses = @([regex]::Matches(
                $workflowSource,
                '(?m)^[ \t]*(?:-[ \t]+)?uses:[ \t]+(?<uses>[^ \t\r\n#]+)'
            ))
        Assert-Equal 2 $actionUses.Count 'Checkout and upload must be the only actions.'
        foreach ($actionUse in $actionUses) {
            Assert-Equal $true (
                $actionUse.Groups['uses'].Value -match
                    '^[^@]+@[0-9a-f]{40}$'
            ) 'Every workflow action must be pinned to a full commit.'
        }
        Assert-Equal $true $workflowSource.Contains(
            'actions/checkout@08eba0b27e820071cde6df949e0beb9ba4906955 # v4.3.0') ''
        Assert-Equal $true $workflowSource.Contains(
            'actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02 # v4.6.2') ''
        Assert-Equal $true $workflowSource.Contains('if-no-files-found: error') ''
        Assert-Equal $true $workflowSource.Contains('fetch-depth: 0') ''
        Assert-Equal $false $workflowSource.Contains('fetch-depth: 1') ''
        Assert-Equal $true $workflowSource.Contains(
            'digest = "sha256:$env:UPLOAD_ARTIFACT_DIGEST"') ''
        Assert-Equal $true $workflowSource.Contains('runHead = $env:ADMISSION_HEAD') ''
        Assert-Equal $true $workflowSource.Contains('Verify durable artifact binding') ''
        Assert-Equal $true $admissionSource.Contains(
            'Worktree bytes do not match the attached raw Git blob') ''
        Assert-Equal $false ([bool]$admissionSchema.additionalProperties) ''
        Assert-Equal $true $assemblerSource.Contains(

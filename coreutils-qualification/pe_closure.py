...
class Image:
    def __init__(self, path):
        ...
        symbols, symbol_count = self.unpack("<II", pe + 12)
        ...
        for index in range(count):
            ...
            if name.startswith("/") and name[1:].isdigit():
                table = symbols + symbol_count * 18
                table_size = self.unpack("<I", table)[0]
                string_offset = int(name[1:])
                if not symbols or not 4 <= string_offset < table_size or table + table_size > len(self.data):
                    raise PeError("Invalid COFF long section name")
                end = self.data.find(b"\0", table + string_offset, table + table_size)
                if end < 0:
                    raise PeError("Unterminated COFF section name")
                name = self.data[table + string_offset:end].decode("ascii")

...
if ($result.Status -cne 'namespace-prepared-not-package-admitted' -or
    $result.Namespace.CC -cne 'gcc' -or
    $result.Namespace.MINGW_PACKAGE_PREFIX -cne 'mingw-w64-aarch64') {
...
if ([regex]::Matches($text, '(?m)^package_git[^ ]* \(\)').Count -ne 16 -or
    ...
    -not $text.Contains('targets="$targets html"') -or
    -not $text.Contains('targets="$targets man"')) {
...
$rejected = $false
try {
    $null = & $prepare -SourceDirectory "$OutputDirectory\mapped" -OutputDirectory "$OutputDirectory\double-mapped"
} catch {
...
$rejected = $false
try {
    $null = & $prepare -SourceDirectory $PinnedRecipe -OutputDirectory "$OutputDirectory\mapped"
} catch {

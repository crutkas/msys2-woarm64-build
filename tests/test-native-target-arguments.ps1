...
New-Item -ItemType Directory -Path "$output\records","$output\bin" | Out-Null
$executable = "$output\bin\arguments.exe"
...
$runs = @()
foreach ($name in @('mingw', 'none', 'mingw-path')) {
    $mode = if ($name -eq 'none') { 'none' } else { 'mingw' }
    $program = if ($name -eq 'mingw-path') { 'arguments.exe' } else { $executable }
    ...
    foreach ($pair in @{
        ...
        PATH="$output\bin;$MsysRoot\usr\bin;$env:SystemRoot\System32"
    ...
    foreach ($arg in @('--noprofile','--norc','-c',
        ...
        'argument-control',$relay,$program,"$output\input with space.txt")) {
    ...
    try {
        ...
        [IO.File]::WriteAllText("$output\$name.stdout.log",$out)
        [IO.File]::WriteAllText("$output\$name.stderr.log",$err)
    } finally {
        if (-not $process.HasExited) { $process.Kill($true); $process.WaitForExit() }
        $process.Dispose()
    }
    $runs += @{Name=$name;Mode=$mode;Program=$program;Exit=$code;Stdout=$out;Stderr=$err}
}
$runs | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath "$output\runs.json" -Encoding utf8
if ($runs[0].Exit -ne 0 -or $runs[0].Stdout.Trim() -cne 'argument-file-ok' -or $runs[1].Exit -ne 1 -or
    $runs[2].Exit -ne 0 -or $runs[2].Stdout.Trim() -cne 'argument-file-ok') {
...
if ($records.Count -ne 3 -or @($records | Where-Object raw_exit -EQ 0).Count -ne 2 -or

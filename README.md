...
The zlib preparer vendors the exact checksum-pinned PR 1165 patch: GitHub's
generated patch changed only its abbreviated index width, breaking downloads
despite unchanged code. Neither the original checksum nor the patch body is
relaxed. Minizip builds beneath the freshly built zlib so its upstream
two-parent-directory library search is correct, with explicit bzip2 dependency
paths. Both zlib/minizip packages, shared/static libraries, demos and checks stay
enabled. `-BootstrapAutotools -BootstrapMsysGenerators` selects real installed
MSYS host generators, not a fabricated native libtool provider.
...
An explicit `WOARM64_JOBS` also bounds XZ/Zstd archive compression to one
thread, preserving configured compression levels and other options. The policy
is applied in an invocation-private libmakepkg copy at archive creation, after
configuration and PKGBUILD overrides, including source-only archives that never
run a build-environment hook. Unknown compressor wrappers and invalid thread
settings fail rather than silently falling back to an uncompressed tar. Legacy
callers without `WOARM64_JOBS` keep their existing compression behavior; the
reusable package workflow exposes `build_jobs` (1-16, default 2). No installed
makepkg configuration or running invocation snapshot is rewritten.

`tests/test-makepkg-compression-policy.sh` covers command normalization and
rejection boundaries. `tests/test-makepkg-compression.sh <fresh-output> [evidence|msys|mingw]`
uses real makepkg and real XZ/Zstd on a tiny compiler-free fixture, checks
package/source extraction and rejection, and compares repeated binary packages at the
same build path with a fixed `SOURCE_DATE_EPOCH`. Different build paths remain
recorded in `.BUILDINFO`; this is not a claim of path-independent package bytes.
Source archive byte identity is not asserted: upstream makepkg also includes
wall-clock timestamps in `.SRCINFO` and source tar headers.
...
Bare executable names are resolved through the calling shell's `PATH` before
the same native PE/root checks; they are not silently treated as files in the
current directory. This preserves libtool's installed-program lookup tests.

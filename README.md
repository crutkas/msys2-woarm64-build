...
The dedicated preview workflow admits only the exact attached commit from this
repository. Its checkout and upload actions are pinned to independently
resolved release commits. Every successful exact-head push or same-repository
pull-request run uploads a uniquely named, closed-schema evidence artifact
containing the 26/26 assembler results, 14/14 ETW results, raw Git blob
identities and SHA-256 values, commit trailers, changed-file manifest,
workflow/action pins, origin-ref bindings, no-private-path result, and
unchanged `C:\msys64` before/after seals. The run fails if the artifact upload
or its API binding is absent.
